                ...
                <Col xs={12} md={4}>
                  {album.title && (
                    <div className="mb-4">
                      ...
                      <Link className="text-light text-decoration-none" to={`/artist/${album.artist.id}`}>
                        {album.artist.name}
                      </Link>
                    </div>
                  )}
                  ...
                  {album.tracks &&
                    album.tracks.data.map((track, index) => (
                      <Row className="justify-content-between py-3 trackHover" key={track.id}>
                        <Col>
                          <h4 className="text-light card-title trackHover px-3">{track.title}</h4>
                        </Col>
                        <Col>
                          <Button onClick={() => handleClick(track, 1)} className="btn btn-success me-1">
                            ❤️
                          </Button>
                          <Button onClick={() => handleClick(track, 0)} className="btn btn-success">
                            🔥
                          </Button>
                        </Col>
                        <Col>
                          <h4 className="text-light card-title trackHover px-3">{(track.duration / 60).toFixed(2)}</h4>
                        </Col>
                      </Row>
                    ))}
                </Col>
